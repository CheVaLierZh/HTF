...
from .base import trmf, trmf_forecast_factors, trmf_reconstruct, trmf_factors_reconstruct
...
class TRMFRegressor(BaseEstimator):
    ...
    def reconstruct(self, X=None):
        if self.fit_regression:
            ...
            X = np.empty((self.factors_.shape[0], 0))
        return trmf_reconstruct(self.loadings_, self.factors_, self.intercept_, self.coef_, regressors=X,
                                mode="exog")

    def reconstruct_blackout(self, X=None):
        if self.fit_regression:
            X = check_array(X, dtype="numeric", accept_sparse=False)
        else:
            X = np.empty((self.factors_.shape[0], 0))

        self.factors_ = trmf_factors_reconstruct(self.ar_coef_, self.factors_)
        return trmf_reconstruct(self.loadings_, self.factors_, self.intercept_, self.coef_, regressors=X,
                                mode="exog")
