...
def trmf_init(data, n_components, n_order, random_state=None):
    ...
    if sp.issparse(data):
        U, s, Vh = svds(data, k=n_components)
...
def trmf_factors_reconstruct(ar_coef, factors):
    n_components, n_order = ar_coef.shape

    if factors.shape[0] < n_order:
        raise TypeError("""Factor history is too short.""")

    recons = factors.copy()
    for h in range(n_order, recons.shape[0], 1):
        recons[h] = np.einsum("il, li->i", ar_coef, recons[h-n_order:h])
...
def trmf_reconstruct(loadings, factors, intercept, beta, regressors=None, mode="exog"):
    n_regressors, n_targets = beta.shape
    if regressors is None:
        if n_regressors > 0:
            raise TypeError("""Regressors must be provided.""")
        regressors = np.empty((factors.shape[0], 0))
    ...
    if mode == "exog":
        if regressors.shape[0] < factors.shape[0]:
            raise TypeError("""Not enough future observations.""")
    ...
    recon = np.dot(factors, loadings)
    if mode == "exog":
        # assume the regressors are exogenous
        targets = np.dot(regressors, beta) + recon + intercept
